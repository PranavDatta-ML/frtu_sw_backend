...
async def validate_unique_module_name(
    ...
    module_types = await FRTUModuleType.select(name=module_type_name)
    if not module_types:
        raise HTTPException(500, f"{module_type_name} module type not configured")
    module_type_id = module_types[0].id
    ...
    device_modules = [
        m for m in modules 
        if (m.attribute or {}).get("device_id") == str(device_id)
    ]
    
    if current_module_id:
        device_modules = [
            m for m in device_modules 
            if str(m.id) != str(current_module_id)
        ]
    
    info_key = "module_di_info" if module_type_name.upper() == "DI" else "module_do_info"
    for module in device_modules:
        existing_name = (
            module.attribute or {}
        ).get(info_key, {}).get("general_info", {}).get("name")
