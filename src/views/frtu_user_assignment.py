...
async def assign_role_to_user(request: Request, authorization: str = Header(...)):
    try:
        if not authorization or not authorization.startswith("Bearer "):
            return HttpStatusCode.NOT_AUTHENTICATED.response(message="Invalid Authorization header")

        token = authorization.split(" ")[1]
        token_payload = decode_access_token(token)

        assigner_user_id = token_payload.get("sub")
        assigner_role_id = token_payload.get("role_id")
        assigner_name = token_payload.get("name")

        if not assigner_user_id or not assigner_role_id:
            return HttpStatusCode.NOT_AUTHENTICATED.response(message="Invalid token payload")

        assigner_user_uuid = uuid.UUID(assigner_user_id)

        assigner_role = await FRTURoles.select(id=uuid.UUID(assigner_role_id))
        if not assigner_role:
            return HttpStatusCode.NOT_AUTHENTICATED.response(message="Invalid assigner role")
        assigner_role_name = assigner_role[0]["name"].lower().strip()

        if assigner_role_name not in ["admin", "super admin"]:
            return HttpStatusCode.NOT_AUTHENTICATED.response(
                message="Only Admin or Super Admin can assign roles"
            )

        payload = await request.json()
        user_id = payload.get("user_id")
        role_id = payload.get("role_id")
        scope_type = (payload.get("scope_type") or "").upper().strip()
        attribute = payload.get("attribute", {})

        if not user_id or not role_id or not scope_type:
            return HttpStatusCode.BAD_REQUEST.response(
                message="Fields 'user_id', 'role_id', and 'scope_type' are required"
            )
        ...
        await FRTUUserAssignment.insert(
            ...
            scope_id=scope_id,
            attribute={
                "assigned_by": str(assigner_user_uuid),
                "assigned_by_name": assigner_name,
                "assigned_by_role": assigner_role_name,
                "extra": attribute
            },
            creation_time=now,
            last_update_time=now
        ...
        return HttpStatusCode.CREATED.response(
            message=f"Role '{assigner_role_name}' assigned successfully to user for '{scope_type}' scope",
            data={
                "assignment_id": str(assignment_id),
