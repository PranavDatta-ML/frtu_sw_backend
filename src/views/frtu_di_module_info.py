...
async def add_di_module_info(
    ...
    if not slots:
        raise HTTPException(
            400,
            "Invalid slot_id or slot does not belong to this device",
        )

    slot_obj = slots[0]
    slot_number = int(slot_obj.name)

    module_types = await FRTUModuleType.select(name="DI")
    if not module_types:
    ...
    if existing_modules:
        ...
        if existing_module.module_type != module_type_id:
            raise HTTPException(
                400,
                "Slot already occupied by another module type",
            )
    ...
    temp_channels = dict(existing_channels)
